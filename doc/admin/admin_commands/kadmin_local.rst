...
{-\|+}\ **lockdown_keys
    **+lockdown_keys** prevents keys for this principal from ever leaving
    the KDC. The chpass and extract operations are denied for a principal
    with this attribute, while the chrand operation is allowed, but will
    not return back the new keys (useful to rotate krbtgt principal keys).
    Other operations like delete and rename are also denied if this attribtue
    is set, in order to prevent a malicious admin from replacing principals
    like krbtgt/* or kadmin/* with others that have compromised keys.
    Tis attribute can be set via the network protocol, but can only be
    removed using the kadmin.local tool.
