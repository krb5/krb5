...
/* Insert an entry into the cache using the given hash value. */
static struct entry *
 ...
 * request is still being processed, then the function will return TRUE with
 * reply_packet_out set to NULL.
 */
krb5_boolean
 ...
 *
 * The reply_packet may be NULL to specify a request that is still processing.
 */
void
