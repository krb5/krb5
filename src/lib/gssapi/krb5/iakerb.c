                ...
                int protocol_err, krb5_data *enc_err)
{
    ...
    /* We must provide a server principal, although we accept the recipient to
...
/* Decode a KRB-ERROR message and return the associated error code. */
static krb5_error_code
iakerb_rd_error(krb5_context context, const krb5_data *enc_err)
{
    ...
    if (error->error > 0 && error->error < 128)
