                         ...
                         int n_key_data)
{
    ...
    if (kvno == 0) {
        ...
        /* check that we do not collide with an existing kvno if we are
         * keeping old kvnos */
        for (i = 0; i < kdb->n_key_data; i++) {
            if (kdb->key_data[i].key_data_kvno == kvno) {
    ...
    if (keepold) {
        ...
        /* Sort the keys as other code in krb5 expect a specific order.
         * We need to sort only if we keep old data, as otherwise we allow
         * only a single kvno to be specified. */
