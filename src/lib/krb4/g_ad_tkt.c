    ...
    int     lifetime;
{
    ...
    ÊÊ if (kerror == GC_NOTKT) {
        ÊÊÊÊÊÊ /* No tickets... call krb_get_cred (KLL will prompt) and try again. */
        ÊÊÊÊÊÊ if ((kerror = krb_get_cred ("krbtgt", realm, realm, &cr)) == KSUCCESS) {
            ÊÊÊÊÊÊÊÊÊÊ /* Now get the realm again. */
            kerror = krb_get_tf_realm (TKT_FILE, lrealm);
        }
    }
#endif
