...
static krb5_error_code
kdb_find_library(krb5_context kcontext, const char *lib_name, db_library *lib)
{
...
krb5_db_load_module(krb5_context kcontext, const char *name)
{
    ...
    ret = kdb_find_library(kcontext, name, &lib);
    if (ret)
        goto cleanup;
...
krb5_db_setup_lib_handle(krb5_context kcontext)
{
    ...
    status = krb5_db_load_module(kcontext, library);
    free(library);
