...
static int stream_or_rpc_data_counter;
 ...
 *
 *
 * This will check if the got socket activated by inspecting two environment
 * variables.
 *
...
static int
kill_lru_stream_or_rpc_connection(void *handle, verto_ev *newev)
{
...
accept_stream_connection(verto_ctx *ctx, verto_ev *ev)
{
    ...
    if (++stream_or_rpc_data_counter > max_stream_data_connections)
        kill_lru_stream_or_rpc_connection(conn->handle, newev);
...
accept_rpc_connection(verto_ctx *ctx, verto_ev *ev)
{
    ...
    for (s = 0; s < FD_SETSIZE; s++) {
        ...
        if (++stream_or_rpc_data_counter > max_stream_data_connections)
            kill_lru_stream_or_rpc_connection(newconn->handle, newev);
    }
}
