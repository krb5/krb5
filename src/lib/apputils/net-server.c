...
enum conn_type {
    CONN_UDP, CONN_TCP_LISTENER, CONN_TCP, CONN_RPC_LISTENER, CONN_RPC, CONN_UNIXSOCK
};
...
struct connection {
    ...
    socklen_t addrlen;
    char addrbuf[108]; /* unix socket len */
...
{
    if (socket_paths == NULL || socket_paths[0] != '/') {
        return EINVAL;
    }
...
accept_unixsock_connection(verto_ctx *ctx, verto_ev *ev)
{
    ...
    flags = VERTO_EV_FLAG_IO_READ | VERTO_EV_FLAG_PERSIST;
    /* TODO FIXME: Do we need process_unixsock_connection_read? */
    ...
    */
    ...
    newconn->remote_addr.address = &newconn->remote_addr_buf;
    /* TODO FIXME: Add support AF_UNIX in init_addr? */
    init_addr(&newconn->remote_addr, ss2sa(&newconn->addr_s));
}
