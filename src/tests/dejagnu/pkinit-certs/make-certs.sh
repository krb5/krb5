...
env SUBJECT=ca   openssl req -config openssl.cnf -new -x509 -extensions exts_ca -set_serial 1 -days $DAYS -key privkey.pem -out ca.pem

# Generate a KDC certificate.
env SUBJECT=kdc  openssl req -config openssl.cnf -new -subj /CN=kdc -key privkey.pem -out kdc.csr
env SUBJECT=kdc  openssl x509 -extfile openssl.cnf -extensions exts_kdc -set_serial 2 -days $DAYS -req -CA ca.pem -CAkey privkey.pem -out kdc.pem -in kdc.csr

# Generate a client certificate and PKCS#12 bundles.
env SUBJECT=user openssl req -config openssl.cnf -new -subj /CN=user -key privkey.pem -out user.csr
env SUBJECT=user openssl x509 -extfile openssl.cnf -extensions exts_client -set_serial 3 -days $DAYS -req -CA ca.pem -CAkey privkey.pem -out user.pem -in user.csr
openssl pkcs12 -export -in user.pem -inkey privkey.pem -out user.p12 -passout pass:
openssl pkcs12 -export -in user.pem -inkey privkey.pem -out user-enc.p12 -passout pass:encrypted
