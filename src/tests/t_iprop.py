...
# Start kpropd for port2.  No -A option is necessary since we're
# talking to the same host as master, but slave2 defines iprop_port to
# $port8 so it will talk to slave1.  Get a full dump from slave1.
kpropd2 = realm.start_server([kpropd, '-d', '-D', '-P', slave2_kprop_port,
                              '-f', slave2_in_dump_path, '-p', kdb5_util,
                              '-a', acl_file], 'ready', slave2)
