...
def wait_for_prop(kpropd, full_expected, expected_old, expected_new):
    ...
    while True:
        ...
        if m:
            ...
            # Also record this as the new sno (whether or not we have
            # seen a full resync), in case we get back UPDATE_NIL.
    ...
    # (This isn't perfect since there's there's a window between the
    # fprintf and the sleep; kpropd will need design changes to fix
    # that.)
    while True:
        line = kpropd.stdout.readline()
        output('kpropd: ' + line)
