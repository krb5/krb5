...
realm = K5Realm(krb5_conf=pkinit_krb5_conf, kdc_conf=pkinit_kdc_conf, get_creds=False)
realm.kinit('user@%s' % realm.realm, flags=['-X', 'X509_user_identity=%s' % file_identity])
realm.klist('user@%s' % realm.realm)
realm.run([kvno, realm.host_princ])
realm.stop()

success('authenticated PKINIT using FILE:-based identity with no key password')

# Run the basic test - PKINIT with FILE: identity, with a password on the key,
# supplied by the prompter.
realm = K5Realm(krb5_conf=pkinit_krb5_conf, kdc_conf=pkinit_kdc_conf, get_creds=False)
realm.kinit('user@%s' % realm.realm, flags=['-X', 'X509_user_identity=%s' % file_enc_identity], password='encrypted')
realm.klist('user@%s' % realm.realm)
realm.run([kvno, realm.host_princ])
realm.stop()

success('authenticated PKINIT using FILE:-based identity with a key password (prompter)')

# PKINIT with DIR: identity, with no password on the key.
realm = K5Realm(krb5_conf=pkinit_krb5_conf, kdc_conf=pkinit_kdc_conf, get_creds=False)
setup_dir_identities(realm)
realm.kinit('user@%s' % realm.realm, flags=['-X', 'X509_user_identity=%s' % p12_identity])
realm.klist('user@%s' % realm.realm)
realm.run([kvno, realm.host_princ])
realm.stop()

success('authenticated PKINIT using DIR:-based identity with no password')

# PKINIT with DIR: identity, with a password on the key, supplied by the
# prompter.
realm = K5Realm(krb5_conf=pkinit_krb5_conf, kdc_conf=pkinit_kdc_conf, get_creds=False)
setup_dir_identities(realm)
realm.kinit('user@%s' % realm.realm, flags=['-X', 'X509_user_identity=%s' % dir_enc_identity], password='encrypted')
realm.klist('user@%s' % realm.realm)
realm.run([kvno, realm.host_princ])
realm.stop()

success('authenticated PKINIT using DIR:-based identity with a password (prompter)')
...
success('authenticated PKINIT using PKCS12:-based identity with no password')

# PKINIT with PKCS12: identity, with a password on the bundle, supplied by the
# prompter.
realm = K5Realm(krb5_conf=pkinit_krb5_conf, kdc_conf=pkinit_kdc_conf, get_creds=False)
realm.kinit('user@%s' % realm.realm, flags=['-X', 'X509_user_identity=%s' % p12_enc_identity], password='encrypted')
realm.klist('user@%s' % realm.realm)
realm.run([kvno, realm.host_princ])
realm.stop()

success('authenticated PKINIT using PKCS12:-based identity with a password (prompter)')

if have_soft_pkcs11:
	os.environ['SOFTPKCS11RC'] = os.path.join(os.getcwd(), 'testdir', 'soft-pkcs11.rc')

	# PKINIT with PKCS11: identity, with a PIN supplied by the prompter.
	realm = K5Realm(krb5_conf=pkinit_krb5_conf, kdc_conf=pkinit_kdc_conf, get_creds=False)
	conf = open(os.environ['SOFTPKCS11RC'], 'w')
	conf.write("%s\t%s\t%s\t%s\n" % ('user', 'user token', user_pem, privkey_enc_pem))
	conf.close()
	realm.kinit('user@%s' % realm.realm, flags=['-X', 'X509_user_identity=%s' % p11_identity], password='encrypted')
	realm.klist('user@%s' % realm.realm)
	realm.run([kvno, realm.host_princ])
	realm.stop()

	success('authenticated PKINIT using PKCS11:-based identity with a PIN (prompter)')
else:
