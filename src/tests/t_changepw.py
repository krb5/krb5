...
# Also listen on unix sockets
unix_conf = {
    'realms': {
        '$realm': {
            'kdc_unixsock_listen': '$testdir/krb5.sock',
            'admin_server': '$hostname:$port1, $testdir/kadmin.sock',
            'kpasswd_server': '$hostname:$port2, $testdir/kpasswd.sock',
        }
    }
}
...
kadmin_unix = realm.special_env('kadmin_unix', True, kdc_conf=unix_conf)
realm.start_kadmind(env=kadmin_unix)
realm.prep_kadmin()
...
mark('password change over uinx domain socket')
