...
main(int argc, char *argv[])
{
     ...
     * first-choice mech), so we have to look directly at the DER of the
     * response token.  If we don't request mutual authentication, the SPNEGO
     * reply will contain no underlying mech token, so the correct response is
     * completely predictable:
     *
     *   A1 14 (choice 1, length 20)
     *     30 12 (sequence, length 18)
     *       A0 03 (context tag 0, length 3)
     *         0A 01 00 (enumerated value 0, meaning accept-complete)
