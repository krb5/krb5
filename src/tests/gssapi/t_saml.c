...
int main(int argc, char *argv[])
{
    ...
    if (argc < 2 || argc > 5) {
        fprintf(stderr, "Usage: %s [--spnego] [assertion-path] "
                        "[proxy-target] [keytab]\n", argv[0]);
    ...
    fbuf = (char *)malloc(statbuf.st_size);
    if (fbuf == NULL) {
	minor = ENOMEM;
	goto out;
    }
    ...
    while (nread != statbuf.st_size) {
	nread += fread(&fbuf[nread], 1, statbuf.st_size, fp);
    }
