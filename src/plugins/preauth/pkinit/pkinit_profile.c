...
/*
 * XXX This is not the correct answer for this function,
 * but it is an example of the kind of thing we need to do.
 */
krb5_error_code
pkinit_get_kdc_hostnames(krb5_context context,
			 krb5_data *realm,
			 char ***hostnames)
{
    profile_t profile;
    ...
    if (context == NULL || realm == NULL || hostnames == NULL)
	return EINVAL;
    ...
    pkiDebug("Searching for '%s' '%s' '%s'\n", names[0], names[1], names[2]);
    retval = profile_get_values(profile, names, &values);
    if (retval)
	pkiDebug("profile_get_values returned %d (%s)\n",
